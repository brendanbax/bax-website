    ...
    <ClientOnly>
        <Teleport to="body">
            <Transition name="overlay-fade">
                <div
                    v-if="showMenu"
                    class="fixed inset-0 z-50 bg-neutral-950/50 backdrop-blur-lg sm:hidden"
                ...
                <div v-if="showMenu" class="fixed top-0 right-0 bottom-0 z-[100] overflow-auto flex flex-col bg-white dark:bg-black w-80 sm:hidden">
                     ...
                     <ul class="py-12 space-y-5 grow">
                        <li v-for="link in links" :key="link.route">
                            <NuxtLink
                                :to="link.route"
                                class="font-bebas text-brand text-3xl pl-6"
                                active-class="border-l-4 border-brand text-neutral-950 dark:text-white"
                                @click="showMenu = false"
                            >{{ link.name }}</NuxtLink>
                        </li>
                     </ul>
